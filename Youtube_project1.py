...
def insert_video_data_mysql(connection, video_data):
    try:
        ...
        cursor = connection.cursor()
        # Construct SQL query
        sql = "INSERT INTO videos (video_id, playlist_id, channel_id, video_name, video_description, published_date, view_count, like_count, dislike_count, favorite_count, comment_count, duration, thumbnail, caption_status) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        # Execute query
        cursor.execute(sql, (video_data['Video_Id'], video_data['Playlist_Id'], video_data['Channel_Id'], video_data['Video_Name'], video_data['Video_Description'], video_data['PublishedAt'], video_data['View_Count'], video_data['Like_Count'], video_data['Dislike_Count'], video_data['Favorite_Count'], video_data['Comment_Count'], video_data['Duration'], video_data['Thumbnail'], video_data['Caption_Status']))
        # Commit changes
        connection.commit()
...
def main():
    ...
    # Connect to MySQL
    connection_mysql = connect_to_mysql()  # Define connection_mysql here
    ...
    if st.button('Retrieve Data') and channel_id:
        try:
            ...
            for playlist in playlists:
                ...
                for video in videos:
                    ...
                    for comment in comments:
                        ...
                        # SQL query to select video names and channel names
            query = """
            ...
            df_videos_channels = execute_sql_query(connection_mysql, query)

            # Display the DataFrame
            st.subheader('Videos and their corresponding channels')
            st.write(df_videos_channels)
    ...
    query_top_10_viewed_videos = """
        SELECT video_name, view_count, channel_name
        FROM videos
        INNER JOIN channels ON videos.channel_id = channels.channel_id
        ORDER BY view_count DESC
        LIMIT 10
    """
    query_video_comments_count = """
    ...
    """
    query_video_likes = """
        SELECT video_name, like_count, channel_name
        FROM videos
        INNER JOIN channels ON videos.channel_id = channels.channel_id
        ORDER BY like_count DESC
        LIMIT 1
    """
    query_video_likes_dislikes = """
    ...
    query_avg_duration = """
        ...
        GROUP BY channel_name
    """
    query_video_comments_max = """
        SELECT video_name, MAX(comment_count) AS max_comments, channel_name
        FROM (
            SELECT video_name, COUNT(*) AS comment_count, channel_name
            FROM videos
            INNER JOIN comments ON videos.video_id = comments.video_id
            INNER JOIN channels ON videos.channel_id = channels.channel_id
            GROUP BY video_name
        ) AS max_comments_table
    """

    # Query results
    df_videos_channels = execute_sql_query(connection_mysql, query_videos_channels)
    df_channel_videos_count = execute_sql_query(connection_mysql, query_channel_videos_count)
    df_top_10_viewed_videos = execute_sql_query(connection_mysql, query_top_10_viewed_videos)
    df_video_comments_count = execute_sql_query(connection_mysql, query_video_comments_count)
    df_video_likes = execute_sql_query(connection_mysql, query_video_likes)
    df_video_likes_dislikes = execute_sql_query(connection_mysql, query_video_likes_dislikes)
    df_channel_views = execute_sql_query(connection_mysql, query_channel_views)
    df_channels_2022 = execute_sql_query(connection_mysql, query_channels_2022)
    df_avg_duration = execute_sql_query(connection_mysql, query_avg_duration)
    df_video_comments_max = execute_sql_query(connection_mysql, query_video_comments_max)

    # Display results
    st.subheader('Names of all videos and their corresponding channels')
    st.write(df_videos_channels)

    st.subheader('Channels with the most number of videos and their counts')
    st.write(df_channel_videos_count)

    st.subheader('Top 10 most viewed videos and their respective channels')
    st.write(df_top_10_viewed_videos)

    st.subheader('Number of comments made on each video and their corresponding video names')
    st.write(df_video_comments_count)

    st.subheader('Videos with the highest number of likes and their corresponding channel names')
    st.write(df_video_likes)

    st.subheader('Total number of likes and dislikes for each video and their corresponding video names')
    st.write(df_video_likes_dislikes)

    st.subheader('Total number of views for each channel and their corresponding channel names')
    st.write(df_channel_views)

    st.subheader('Names of all the channels that have published videos in the year 2022')
    st.write(df_channels_2022)
    ...
    st.write(df_video_comments_max)
